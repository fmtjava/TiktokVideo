...
/**
 *  自定义 FragmentNavigator：使用 add、hide、show 替换 replace，这里注意，每个 Navigation 库中的 FragmentNavigator 实现可能不一致
 *                           若使用的 Navigation 版本与本项目不一致，可能需要自动改造
 */
...
public open class FixFragmentNavigator(
...
) : Navigator<FixFragmentNavigator.Destination>() {
    ...
    // Add a flag to track navigation state
        ...
        fragmentManager.addOnBackStackChangedListener(
            object : OnBackStackChangedListener {
                ...
                override fun onBackStackChangeStarted(fragment: Fragment, pop: Boolean) {
                    Log.d(TAG, "onBackStackChangeStarted: fragment=${fragment.tag}, pop=$pop")
                ...
                override fun onBackStackChangeCommitted(fragment: Fragment, pop: Boolean) {
                    Log.d(TAG, "onBackStackChangeCommitted: fragment=${fragment.tag}, pop=$pop")
                    val entry =
                    ...
                    if (!pop && !popOp) {
                        if (!fragment.isRemoving && entry == null) {
                            val isInitialFragment = state.backStack.value.isEmpty() &&
                                    fragmentManager.fragments.contains(fragment)

                            val isOurFragment = fragment.tag?.let { tag ->
                                state.backStack.value.any { it.id == tag } ||
                                        state.transitionsInProgress.value.any { it.id == tag }
                            } ?: false

                            if (!isInitialFragment && !isOurFragment) {
                                Log.w(
                                    TAG,
                                    "The fragment $fragment is unknown to the FragmentNavigator. " +
                                            "This may happen if the fragment was added directly to the FragmentManager."
                                )
                            }
                        }
                    }
                    ...
                    if (entry != null) {
                        ...
                        if (isLoggingEnabled(Log.VERBOSE)) {
                            Log.v(
                                TAG,
                                "Fragment $fragment is being removed but has no associated entry"
                            )
                        }
                    }
                }
    ...
    override fun popBackStack(popUpTo: NavBackStackEntry, savedState: Boolean) {
        ...
        if (savedState) {
            ...
            // 显示目标 Fragment
            val destination = popUpTo.destination as Destination
    ...
    private fun navigate(
        ...
        Log.d(TAG, "navigate: entry=${entry.id}, navOptions=$navOptions")
    ...
    private fun createFragmentTransaction(
        ...
        if (fragment == null) {
            try {
                ...
                if (isLoggingEnabled(Log.VERBOSE)) {
                    Log.v(TAG, "Created new fragment instance: $fragment with tag: $tag")
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to instantiate fragment: $className", e)
                throw e
            }
        } else if (isLoggingEnabled(Log.VERBOSE)) {
            Log.v(TAG, "Reusing existing fragment: $fragment with tag: $tag")
        }
        ...
        // 使用 add、show、hide 替换原先的 replace 方式，放在切换时闪烁问题
        if (!fragment.isAdded) {
            ft.add(containerId, fragment, tag)
            if (isLoggingEnabled(Log.VERBOSE)) {
                Log.v(TAG, "Adding new fragment to container: $fragment")
            }
        }
    ...
    public constructor(fragmentNavigator: Navigator<out Destination>) :
        ...
        ) : this(navigatorProvider.getNavigator(FixFragmentNavigator::class.java))
